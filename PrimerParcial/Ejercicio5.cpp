...
// Función que determina si un número tiene una cantidad par de dígitos
bool tieneCantidadParDigitos(int n) {
    int cantidad = 0;
    while (n != 0) {
        cantidad++;
        n /= 10;
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        int numero;
        std::cin >> numero;
        if (tieneCantidadParDigitos(numero)) {
            numeros[i] = numero;
        }
    }
    ordenarDescendente(numeros, n);
    std::cout << "Los números con cantidad par de dígitos ordenados de forma descendente son:" << std::endl;
    for (int i = 0; i < n; i++) {
        if (numeros[i] != 0) {
